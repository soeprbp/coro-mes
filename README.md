...
integration/
├── CoroMES.Integration.Alerts       # Active guarded alerting boundary
...
Unit tests currently cover the CTI ingestion framework and i3X client behavior. Integration tests cover the Blazor host auth gate, equipment audit writes, persisted display definitions, UpKeep sync audit logging, and the alarm lifecycle.
